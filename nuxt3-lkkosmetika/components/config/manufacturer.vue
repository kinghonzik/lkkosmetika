<template>
  <div>
    <div
      v-if="config && 'manufacturers' in config"
    ...
    >
      ...
      <div class="content">
        <table class="table">
          ...
          <tbody>
            <tr v-for="item of config.manufacturers">
              <td>{{ item.id }}</td>
              <td>{{ item.title }}</td>
              <td class="controls">
                <button
              ...
              <td>
                <span class="matches" v-if="categoryCount(item) > 0">
                  {{ categoryCount(item) }} kategorií
                </span>
              </td>
            </tr>
          </tbody>
...
export default {
  ...
  methods: {
    ...
    productsCount(manufacturerObj) {
      if (!this.products) return 0;

      const relProducts = this.products.filter(
        (itm) => itm.data.manufacturer == manufacturerObj.id
      );
    ...
    categoryCount(manufacturerObj) {
      if (!this.config.categories) return 0;

      const relCats = this.config.categories.filter(
        (itm) => itm.manufacturer == manufacturerObj.id
      );

      if (relCats.length > 0) return relCats.length;
    ...
    add() {
      if (!this.checkID(this.newObj.id, this.config.manufacturers)) return;
      if (!this.checkTitle(this.newObj.title, this.config.manufacturers)) return;
      this.config.manufacturers.push(this.newObj);
      this.newObj = null;
      this.dialogType = null;
      this.configSaveAndReload();
    },
    del(item) {
      const cat = this.config.categories.find((itm) => itm.manufacturer == item.id);
      if (cat) {
        alert(
          "Nelze smazat - tento výrobce byl nalezen u kategorie s názvem" + cat.title
        );
        return;
      }
      const product = this.products.find((itm) => itm.data.manufacturer == item.id);
      if (product) {
        alert("Tento výrobce byl nalezen u produktu: " + product.data.title);
        return;
      }
      this.config.manufacturers = this.config.manufacturers.filter(
        (itm) => itm.id != item.id
      );
      this.dialogType = null;
      this.configSaveAndReload();
    ...
    up(item) {
      const index = this.config.manufacturers.indexOf(item);
      if (index > 0) {
        const itm1 = this.config.manufacturers[index - 1];
        const itm2 = this.config.manufacturers[index];

        this.config.manufacturers[index - 1] = itm2;
        this.config.manufacturers[index] = itm1;
        this.configSaveAndReload();
      }
    },
    down(item) {
      const index = this.config.manufacturers.indexOf(item);
      if (index < this.config.manufacturers.length - 1) {
        const itm1 = this.config.manufacturers[index];
        const itm2 = this.config.manufacturers[index + 1];

        this.config.manufacturers[index + 1] = itm1;
        this.config.manufacturers[index] = itm2;
        this.configSaveAndReload();
      }
    },
  },
